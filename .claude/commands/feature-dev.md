...
## 命令说明
这是一个**自动应用**的标准化功能开发流程，适用于所有新功能开发和功能增强。

**重要说明：**
- ✅ **自动触发**：当用户需求满足触发条件时，Claude 应**自动**应用此流程，无需用户手动调用 `/feature-dev` 命令
- ✅ **强制执行**：此流程是强制性的，不可跳过任何步骤
- ✅ **优先级最高**：此流程优先级高于其他通用开发流程

## 触发条件（自动检测）
当用户提出以下类型的需求时，Claude 应**自动且强制**应用此流程：
...
- 系统集成
- 任何涉及代码修改的需求
- 包含"新增"、"添加"、"实现"、"开发"等关键词的需求

### 隐式触发（需判断）
- 用户描述了一个具体的功能需求（即使没有明确说"新增"）
- 用户要求修改现有功能的行为
- 用户要求优化或重构代码
...
- v1.1.0 (2026-01-14):
  - 添加元数据部分，标记为自动应用流程
  - 强化触发条件说明，增加明确触发、隐式触发和不触发的分类
  - 明确说明 Claude 应自动应用此流程，无需用户手动调用命令
  - 提升流程优先级，标记为强制执行
