...
class BinanceAdapter(ExchangeInterface):
    ...
    def connect(self) -> bool:
        ...
        try:
            self.exchange = ccxt.binance({
                ...
                "options": {
                    "defaultType": "future",  # Binance使用future而不是swap
                }
            })
            ...
            logger.info("Binance交易所连接成功")
            return True

        except ccxt.AuthenticationError as e:
            logger.error(f"Binance认证失败: {e}")
            raise AuthenticationError(f"Binance认证失败: {e}", e)
        except Exception as e:
            logger.error(f"Binance连接失败: {e}")
            raise ExchangeError(f"Binance连接失败: {e}", e)
    ...
    def _setup_trading_params(self):
        ...
        try:
            ...
            # 设置保证金模式（Binance使用CROSSED或ISOLATED）
            margin_mode_map = {
                "crossed": "CROSSED",
                "isolated": "ISOLATED"
            }
            binance_margin_mode = margin_mode_map.get(self.margin_mode.lower(), "CROSSED")

            self.exchange.set_margin_mode(binance_margin_mode, self.symbol)

            logger.info(f"Binance交易参数设置成功: 杠杆{self.leverage}x, 保证金模式{binance_margin_mode}")

        except Exception as e:
            logger.warning(f"Binance设置交易参数失败: {e}")

    def disconnect(self):
        """断开连接"""
        self.exchange = None
        logger.info("Binance交易所连接已断开")
    ...
    def get_ticker(self, symbol: str = None) -> Optional[TickerData]:
        ...
        try:
            ...
            logger.error(f"Binance获取行情网络错误: {e}")
            raise NetworkError(f"获取行情网络错误: {e}", e)
        except ccxt.RateLimitExceeded as e:
            logger.error(f"Binance限流: {e}")
            raise RateLimitError(f"限流: {e}", e)
        except Exception as e:
            logger.error(f"Binance获取行情失败: {e}")
    ...
    def get_klines(self, symbol: str = None, timeframe: str = None,
        ...
        try:
            ...
            logger.error(f"Binance获取K线网络错误: {e}")
            raise NetworkError(f"获取K线网络错误: {e}", e)
        except Exception as e:
            logger.error(f"Binance获取K线失败: {e}")
    ...
    def get_balance(self) -> float:
        ...
        try:
            ...
            logger.error(f"Binance余额不足: {e}")
            raise InsufficientBalanceError(f"余额不足: {e}", e)
        except Exception as e:
            logger.error(f"Binance获取余额失败: {e}")
    ...
    def get_positions(self, symbol: str = None) -> List[PositionData]:
        ...
        try:
            ...
            logger.error(f"Binance获取持仓失败: {e}")
    ...
    def open_long(self, amount: float, df: pd.DataFrame = None, **kwargs) -> OrderResult:
        """开多单"""
        return self._create_order("buy", amount, position_side="LONG", reduce_only=False, **kwargs)

    def open_short(self, amount: float, df: pd.DataFrame = None, **kwargs) -> OrderResult:
        """开空单"""
        return self._create_order("sell", amount, position_side="SHORT", reduce_only=False, **kwargs)

    @retry_on_error(max_retries=3, backoff_base=1.0)
    def _create_order(self, side: str, amount: float, position_side: str = None,
        ...
        try:
            ...
            logger.info(f"Binance订单创建成功: {side} {amount}")
            ...
            logger.error(f"Binance订单无效: {e}")
            return OrderResult(success=False, error=f"订单无效: {e}")
        except Exception as e:
            logger.error(f"Binance创建订单失败: {e}")
    ...
    def close_position(self, reason: str = "", position_data: Dict = None) -> bool:
        ...
        try:
            # Binance平仓：使用相反方向的订单 + reduceOnly
            close_side = "sell" if position_side == 'long' else "buy"
            binance_position_side = "LONG" if position_side == 'long' else "SHORT"
            ...
            if result.success:
                logger.info(f"Binance平仓成功: {position_side}, 原因: {reason}")
                return True
            else:
                logger.error(f"Binance平仓失败: {result.error}")
                return False

        except Exception as e:
            logger.error(f"Binance平仓失败: {e}")
            return False
    ...
    def close_all_positions(self) -> List[OrderResult]:
        ...
        try:
            ...
            logger.error(f"Binance一键平仓所有持仓失败: {e}")
    ...
    def set_leverage(self, leverage: int, symbol: str = None) -> bool:
        ...
        try:
            ...
            logger.info(f"Binance设置杠杆成功: {leverage}x")
            return True

        except Exception as e:
            logger.error(f"Binance设置杠杆失败: {e}")
            return False
    ...
    def set_margin_mode(self, mode: str, symbol: str = None) -> bool:
        ...
        try:
            # Binance使用CROSSED或ISOLATED
            margin_mode_map = {
                "crossed": "CROSSED",
                "isolated": "ISOLATED"
            }
            binance_margin_mode = margin_mode_map.get(mode.lower(), "CROSSED")

            self.exchange.set_margin_mode(binance_margin_mode, symbol)

            self.margin_mode = mode
            logger.info(f"Binance设置保证金模式成功: {binance_margin_mode}")
            return True
