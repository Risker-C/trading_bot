...
**优化内容**：
- 降低盈利门槛：从 0.15 USDT 降低到 0.012 USDT
- 让动态止盈机制更容易启用
- 仅覆盖手续费成本，避免门槛过高导致功能无法使用

**问题分析**：
- 原门槛 0.15 USDT 对小仓位（~10 USDT）过高
- 实际案例：净盈利 0.006 USDT，需要增长 25倍才能达到门槛
- 导致动态止盈机制几乎无法启用

**优化效果**：
- 新门槛 0.012 USDT 仅覆盖手续费成本
- 更容易启用动态止盈保护
- 适合小额交易场景
...
```python
# 优化前
MIN_PROFIT_THRESHOLD_USDT = 0.15  # 过高

# 优化后
MIN_PROFIT_THRESHOLD_USDT = 0.012  # 仅覆盖手续费
```

### v1.1.0 (2025-12-17)

**优化内容**：
- 优化回撤阈值：从 0.5% 降低到 0.1%
- 提高对小仓位的敏感度，更及时地捕捉止盈机会
- 解决小仓位盈利大幅回撤仍未触发止盈的问题

**问题分析**：
- 原阈值 0.5% 对小仓位过于宽松
- 实际案例：盈利从峰值回撤 67.8% 仍未触发
- 需要价格下跌 433 USDT 才能触发，但最大盈利仅 0.0177 USDT

**优化效果**：
- 新阈值 0.1% 更适合小仓位交易
- 价格只需下跌约 86.6 USDT 即可触发
- 大幅提高止盈触发的及时性
...
# 优化后
TRAILING_TP_FALLBACK_PERCENT = 0.001  # 0.1%
```
